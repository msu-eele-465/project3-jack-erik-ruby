...
void init_LED(struct status_LED sl)
{
    ...
    uint16_t *PX_SEL0 = (uint16_t *) (sl.led_port_base_addr + OFS_P1SEL0); 
    uint16_t *PX_DIR = (uint16_t *) (sl.led_port_base_addr + OFS_P1DIR); 
...
{
    sl->current_state = s;
    TB3CTL &= ~MC_UP;  // setting MC=0 to stop timer
   ...
   switch (s) 
   {
    ...
    TB3CTL |= MC_UP;  // start timer again in up mode
}
